...
double setpoint = 100;        // Tốc độ mục tiêu (RPM)
...
double mu[NUM_NEURONS] = {0, 50, 100};  // Trung tâm neuron cho các giá trị sai số
double sigma[NUM_NEURONS] = {25, 25, 25};  // Độ rộng của hàm Gaussian
...
void loop() {
  ...
  // In giá trị sai số
  //Serial.print("Error (e): "); Serial.println(e);
  ...
  Serial.print(currentSpeed);  // In tốc độ hiện tại
  // Nếu bạn muốn in cả giá trị setpoint
  ...
  // Serial.println(currentSpeed);  // In tốc độ hiện tại
  // Serial.print(" Control Signal: "); Serial.println(controlSignal);

  // // In ra các giá trị PID để kiểm tra
  // Serial.print("Kp: "); Serial.println(Kp);
  // Serial.print("Ki: "); Serial.println(Ki);
  // Serial.print("Kd: "); Serial.println(Kd);
  
  // In giá trị phi để kiểm tra
  double phi[NUM_NEURONS];
  double totalPhi = 0; // Tổng giá trị phi để chuẩn hóa
  for (int i = 0; i < NUM_NEURONS; i++) {
      phi[i] = exp(-pow(e - mu[i], 2) / (2 * pow(sigma[i], 2)));
      totalPhi += phi[i];  // Cộng dồn để chuẩn hóa
      // Serial.print("phi["); Serial.print(i); Serial.print("]: ");
      // Serial.println(phi[i]);
  }

  // Serial.print("totalPhi: "); Serial.println(totalPhi);
...
void initializeWeights() {
  // Khởi tạo trọng số mạng RBF với giá trị không phải 0
  weights[0][0] = 1.0;  // Trọng số cho Kp từ neuron 1
  weights[0][1] = 0.5;  // Trọng số cho Ki từ neuron 1
  weights[0][2] = 0.2;  // Trọng số cho Kd từ neuron 1

  weights[1][0] = 0.8;  // Trọng số cho Kp từ neuron 2
  weights[1][1] = 0.4;  // Trọng số cho Ki từ neuron 2
  weights[1][2] = 0.1;  // Trọng số cho Kd từ neuron 2

  weights[2][0] = 0.5;  // Trọng số cho Kp từ neuron 3
  weights[2][1] = 0.2;  // Trọng số cho Ki từ neuron 3
  weights[2][2] = 0.05; // Trọng số cho Kd từ neuron 3
}
